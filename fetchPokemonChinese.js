...
export default async (event) => {
  try {
    ...
    const { data } = await axios.get('https://zh.wikipedia.org/wiki/%E5%AE%9D%E5%8F%AF%E6%A2%A6%E5%88%97%E8%A1%A8')
    // 轉成 JQ 可用型式
    const $$ = cheerio.load(data)
    let pokemonNumber = ''
    // 透過使用者輸入的中文來配對它的編號
    $$('.mw-collapsible td').each(function (index) {
      if ($$('.mw-collapsible td').eq(index).text().trim().split(' ').filter(t => t.length > 1 && !t.match(/[a-z]/g)).toString() === event.message.text) {
        pokemonNumber = $$('.mw-collapsible td').eq(index - 1).text().trim()
      }
    })

    // 編號資料抓取
    const { data: pokemon } = await axios.get(`https://tw.portal-pokemon.com/play/pokedex/${pokemonNumber}`)
    const $ = cheerio.load(pokemon)
    // 寶可夢名字
    ...
    // 上方圖片網址導向
    bubble.hero.action.uri = `https://tw.portal-pokemon.com/play/pokedex/${pokemonNumber}`
    // 放入寶可夢資訊
